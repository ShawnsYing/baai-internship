...
问题：flag_gems 以 editable 模式全局安装于 /home/shuang/FlagGems/src/ 下，
...
方式 1：作为 Python 入口脚本（推荐，最可靠）
  python3 /home/shuang/baai-internship/auto_gen/fix_worktree_import.py -c "import flag_gems; print(flag_gems.__file__)"
  python3 /home/shuang/baai-internship/auto_gen/fix_worktree_import.py --pytest tests/test_binary_pointwise_ops.py -m div -vs

方式 2：通过 exec + 分隔符嵌入（不引入路径污染）
  python3 -c "
import sys; sys.path.insert(0, '/home/shuang/baai-internship/auto_gen')
exec(open('/home/shuang/baai-internship/auto_gen/fix_worktree_import.py').read().split('SPLIT_HERE')[0])
...
方式 3：通过 PYTHONSTARTUP 环境变量
  PYTHONSTARTUP=/home/shuang/baai-internship/auto_gen/fix_worktree_import.py python3 -c "import flag_gems; print(flag_gems.__file__)"
"""
import os
import sys
...
# 1. Remove ALL /home/shuang/FlagGems paths from sys.path (from global editable install)
sys.path = [p for p in sys.path if "/home/shuang/FlagGems" not in p]
